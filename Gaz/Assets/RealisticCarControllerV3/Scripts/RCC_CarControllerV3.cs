...
public class RCC_CarControllerV3 : RCC_Core {
	...
	[HideInInspector]public bool cutGas = true;
	private bool permanentGas = true;
	#endregion
	...
	void Update (){
		...
		if (throttleInput < .1f && transform.InverseTransformDirection(rigid.velocity).z > -1f && direction == -1 && !changingGear && automaticGear && !semiAutomaticGear)
	...
	*/
	...
	private void Engine (){
		...
		engineRPM = Mathf.SmoothDamp(engineRPM, (Mathf.Lerp(minEngineRPM, maxEngineRPM + 500f, (clutchInput * throttleInput)) + ((Mathf.Abs(wheelRPM / Mathf.Clamp(poweredWheels, 1, Mathf.Infinity)) * finalRatio * (gears[currentGear].maxRatio)) * (1f - clutchInput))), ref velocity , engineInertia);
